...
def AMMDRPGSTSINC(datos): #, vals_xL, vals_xR):
    ...
    T_index = range(2*datos.m + 2)
    T_index_prima = range(1, 2*datos.m+1)
    T_index_primaprima = range(2*datos.m+1)
    D_index = range(nD)
    ...
    for t1 in T_index_prima:
        for t2 in T_index_prima:
            ...
            if t1 <= t2:    
                ...
                MODEL.addConstrs(gp.quicksum(dLRt[t]/datos.vC for t in T_index_prima if t >= t1 and t <= t2)  + gp.quicksum(dRLt[t]/datos.vC for t in T_index_prima if t >= t1 and t < t2)<= Capacity + BigM*(2- gp.quicksum(uigtd[i, g, t1, d] for i in grafos[g-1].aristas) - gp.quicksum(vigtd[i, g, t2, d] for i in grafos[g-1].aristas)) for g in T_index_prima for d in D_index)               
