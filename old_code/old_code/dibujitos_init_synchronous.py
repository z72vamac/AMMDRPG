...
data = pd.read_csv('synchronous_results_with.csv')
data = data[
    ['Size', 'Instance', 'Alpha_e', 'time_endurance', 'fleet_size', 'GAP', 'Runtime', 'NodeCount', 'ObjVal', 'HeurTime',
     'HeurVal']]
# data['Runtime'] = np.log(data['Runtime'])


data['Difference'] = (data['HeurVal'] - data['ObjVal']) / (data['HeurVal']) * 100
...
data[['Size', 'Instance', 'time_endurance', 'fleet_size']] = data[['Size', 'Instance', 'time_endurance', 'fleet_size']].apply(
    np.int64)

sns.set(style="darkgrid")

g = sns.catplot(x='Size', y='Difference', kind='box', col='fleet_size', hue='Alpha_e', data=data, aspect=1,
                sharey=True, legend=True)

tikzplotlib.save('gap_between_algorithms_synchronous.tex')
plt.savefig('gap_between_algorithms_synchronous.png')

# data = pd.read_csv('AMMDRPG_results_heatmap.csv')
#
# print(data)
#
