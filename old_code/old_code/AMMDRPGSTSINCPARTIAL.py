...
def AMMDRPGSTSINCPARTIAL(data, clusters):  # , vals_xL, vals_xR):
    ...
    for k in clusters.keys():
            ...
            for t2 in T_index_prima:
                if t1 < t2:
                    MODEL.addConstrs((gp.quicksum(
                        uigtd[i, g1, t, d] for g1 in G_index for i in graphs[g1 - 1].edges for t in T_index_prima if
                        t > t1 and t <= t2) <= 10 * (2 - uigtd.sum('*', g, t1, d) - vigtd.sum('*', g, t2, d)) for g in
                                      G_index for d in D_index), name="syncronized1")
                    MODEL.addConstrs((gp.quicksum(
                        vigtd[i, g1, t, d] for g1 in G_index for i in graphs[g1 - 1].edges for t in T_index_prima if
                        t >= t1 and t < t2) <= 10 * (2 - uigtd.sum('*', g, t1, d) - vigtd.sum('*', g, t2, d)) for g in
                                      G_index for d in D_index), name="syncronized2")

                # if t1 == t2:
                # MODEL.addConstrs((gp.quicksum(uigtd[i, g, t, d] for i in graphs[g-1].edges for t in T_index_prima if t > t1 and t <= t2) <= 2 - uigtd.sum('*', g, t1, d) - vigtd.sum('*', g, t2, d) for g in G_index for d in D_index), name = "syncronized1")
                # MODEL.addConstrs((gp.quicksum(vigtd[i, g, t, d] for i in graphs[g-1].edges for t in T_index_prima if t >= t1 and t < t2) <= 2 - uigtd.sum('*', g, t1, d) - vigtd.sum('*', g, t2, d) for g in G_index for d in D_index), name = "syncronized2")

                if t1 <= t2:
                    MODEL.addConstrs((gp.quicksum(pLigtd[i, g, t1, d] for i in graphs[g - 1].edges) + pigjg.sum('*',
                                                                                                                  '*',
                                                                                                                  g) + gp.quicksum(
                        pig[i, g] * graphs[g - 1].edges_length[i // 100 - 1, i % 100] for i in
                        graphs[g - 1].edges) + gp.quicksum(
                        pRigtd[i, g, t2, d] for i in graphs[g - 1].edges)) / data.drone_speed <= gp.quicksum(
                        ...
                        uigtd[i, g, t1, d] for i in graphs[g - 1].edges) - gp.quicksum(
                        vigtd[i, g, t2, d] for i in graphs[g - 1].edges)) for g in G_index for d in D_index)
                    MODEL.addConstrs(
                        gp.quicksum(dLRt[t] / data.truck_speed for t in T_index_prima if t >= t1 and t <= t2) + gp.quicksum(
                            dRLt[t] / data.truck_speed for t in T_index_prima if t >= t1 and t < t2) <= time_endurance + BigM * (
                                    2 - gp.quicksum(uigtd[i, g, t1, d] for i in graphs[g - 1].edges) - gp.quicksum(
                                vigtd[i, g, t2, d] for i in graphs[g - 1].edges)) for g in G_index for d in D_index)
        ...
        # MODEL.addConstrs(gp.quicksum(uigtd[i, g, t1, d] for i in graphs[g-1].edges for t1 in T_index_prima if t1 <= t) - gp.quicksum(vigtd[i, g, t1, d] for i in graphs[g-1].edges for t1 in T_index_prima if t1 >= t) == 0 for g in G_index for d in D_index for t in T_index_prima)
        MODEL.addConstrs(uigtd.sum('*', g, t, d) <= gp.quicksum(
            vigtd[i, g, t1, d] for i in graphs[g - 1].edges for t1 in T_index_prima if t1 >= t) for t in T_index_prima
                         for g in G_index for d in D_index)
        # MODEL.addConstrs(vigtd.sum('*', g, t, d) <= gp.quicksum(uigtd[i, g, t1, d] for i in graphs[g-1].edges for t1 in T_index_prima if t1 > t) for t in T_index_prima for g in G_index for d in D_index)
        # MODEL.addConstrs(vigtd.sum('*', g, t, d) >= gp.quicksum(uigtd[i, g, t1, d] for i in graphs[g-1].edges for t1 in T_index_prima if t1 < t) for t in T_index_prima for g in G_index for d in D_index)
        ...
        for g in G_index:
            for i in graphs[g - 1].edges[0:]:
                for j in graphs[g - 1].edges[0:]:
                    if i != j:
                        MODEL.addConstr(
                            graphs[g - 1].edges_number - 1 >= (sig[i, g] - sig[j, g]) + graphs[g - 1].edges_number *
                            zigjg[i, j, g])
        ...
        for i, g in rhoig.keys():
            ...
            MODEL.addConstr(
                Rig[i, g, 0] == rhoig[i, g] * graphs[g - 1].V[first, 0] + (1 - rhoig[i, g]) * graphs[g - 1].V[
                    second, 0])
            MODEL.addConstr(
                Rig[i, g, 1] == rhoig[i, g] * graphs[g - 1].V[first, 1] + (1 - rhoig[i, g]) * graphs[g - 1].V[
                    second, 1])
            MODEL.addConstr(
        ...
        if not (data.alpha):
            for g in G_index:
                MODEL.addConstr(gp.quicksum(
                    pig[i, g] * graphs[g - 1].edges_length[i // 100 - 1, i % 100] for i in graphs[g - 1].edges) >=
                                graphs[g - 1].alpha * graphs[g - 1].length)
        ...
        # objective = gp.quicksum(pLigtd[i, g, t, d] + pRigtd[i, g, t, d] for i, g, t, d in pRigtd.keys()) + gp.quicksum(pigjg[i, j, g] for i, j, g in pigjg.keys()) + gp.quicksum(pig[i, g]*graphs[g-1].edges_length[i // 100 - 1, i % 100] for g in G_index for i in graphs[g-1].edges) + gp.quicksum(3*dLRt[t] for t in dLRt.keys()) + gp.quicksum(3*dRLt[t] for t in dRLt.keys())
