...
point_gurobi = pd.read_csv('results_point_gurobi_without_initialization.csv')
point_gurobi = point_gurobi[['Size', 'Instance', 'Capacity', 'GAP', 'NodeCount', 'Runtime', 'ObjVal']]
# point_gurobi['Runtime'] = np.log(point_gurobi['Runtime'])
point_gurobi['Type'] = 'Point_Gurobi'

polygonal_gurobi = pd.read_csv('results_polygonal_gurobi_without_initialization.csv')
polygonal_gurobi = polygonal_gurobi[['Size', 'Instance', 'Capacity', 'GAP', 'NodeCount', 'Runtime', 'ObjVal']]
# polygonal_gurobi['Runtime'] = np.log(polygonal_gurobi['Runtime'])
polygonal_gurobi['Type'] = 'Polygonal_Gurobi'
...
polygonal_cplex['Type'] = 'Polygonal_Cplex'

# print(np.sum(polygonal_cplex['Runtime']))
...
# g = sns.catplot(x = 'Size', y = 'GAP', hue = 'Type', kind = 'box', data = comparador, aspect = 0.3, sharey = True, legend = True)
#
# plt.show()

# todo = pd.concat([point_gurobi, polygonal_gurobi])
# todo[['Size', 'Instance', 'Capacity']] = todo[['Size', 'Instance', 'Capacity']].apply(np.int64)
# g = sns.catplot(x = 'Size', y = 'GAP', kind = 'box', hue = 'Type', data = todo, aspect = 1, sharey = True, legend = True)

# import tikzplotlib
#
# matplotlib.rcParams['axes.unicode_minus'] = False
#
# tikzplotlib.save('gap_gurobi_without_initialization.tex', encoding = 'utf-8')
#
# plt.savefig('gap_gurobi_without_initialization.png')
#
# todo = pd.concat([point_cplex, polygonal_cplex])
# todo[['Size', 'Instance', 'Capacity']] = todo[['Size', 'Instance', 'Capacity']].apply(np.int64)
# g = sns.catplot(x = 'Size', y = 'GAP', kind = 'box', hue = 'Type', data = todo, aspect = 1, sharey = True, legend = True)
#
# tikzplotlib.save('gap_cplex_without_initialization.tex')
# plt.savefig('gap_cplex_without_initialization.png')
...
# print(datos)
#
# valores = datos.pivot("Capacity", "Num_Drones", "ObjVal")
# ax = sns.heatmap(valores)
