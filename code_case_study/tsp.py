...
def tsp(Ck):
    ...
    restricciones = MODEL.addConstrs(gp.quicksum(zkk[v, w] for v, w in permutations(set, 2)) <= len(set) - 1 for set in list(powerset(K_index)) if len(set) < len(K_index))
    restricciones.Lazy = 3
    
    
    objective = gp.quicksum(np.linalg.norm(np.array([Ck[(k1, 0)], Ck[(k1, 1)]]) - np.array([Ck[(k2, 0)], Ck[(k2, 1)]]))*zkk[k1, k2]*14000/1e6 for k1, k2 in KK_index)
