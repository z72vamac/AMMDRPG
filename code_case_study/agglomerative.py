...
def agglomerative(data, paths, scale = 1):
    ...
    def can_be_joined(list):
        ...
        MODEL.addConstrs(dif_L_g[g, dim] >= (paths_problem[g][1][1][dim] - centroid[dim]) * 14000 / 1e6 for g, dim in dif_L_g.keys())
        MODEL.addConstrs(dif_L_g[g, dim] >= (-paths_problem[g][1][1][dim] + centroid[dim]) * 14000 / 1e6 for g, dim in dif_L_g.keys())
        ...
        MODEL.addConstrs(dif_R_graph[g, dim] >= (paths_problem[g][1][0][dim] - centroid[dim]) * 14000 / 1e6 for g, dim in dif_R_graph.keys())
        MODEL.addConstrs(dif_R_graph[g, dim] >= (-paths_problem[g][1][0][dim] + centroid[dim]) * 14000 / 1e6 for g, dim in dif_R_graph.keys())
