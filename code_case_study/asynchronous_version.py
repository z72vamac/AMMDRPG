...
def ASYNCHRONOUS(datos):
    ...
    for o in O_index:
        for g in G_index:
            for e in grafos[g-1].aristas:
                for dim in range(2):
                    MODEL.addConstr(difLego[e, g, o, dim] >=  (xLo[o, dim] - Reg[e, g, dim])*14000/1e6)
                    MODEL.addConstr(difLego[e, g, o, dim] >= (-xLo[o, dim] + Reg[e, g, dim])*14000/1e6)
    ...
    for o in O_index:
        for g in G_index:
            for e in grafos[g-1].aristas:
                for dim in range(2):
                    MODEL.addConstr(difRego[e, g, o, dim] >=  (Leg[e, g, dim] - xRo[o, dim])*14000/1e6)
                    MODEL.addConstr(difRego[e, g, o, dim] >= (-Leg[e, g, dim] + xRo[o, dim])*14000/1e6)
    ...
    for g in G_index:
        for e in grafos[g-1].aristas:
            for e_prima in grafos[g-1].aristas:
                if e != e_prima:
                    for dim in range(2):
                        MODEL.addConstr(difegeg[e, e_prima, g, dim] >=  (Leg[e, g, dim] - Reg[e_prima, g, dim])*14000/1e6)
                        MODEL.addConstr(difegeg[e, e_prima, g, dim] >= (-Leg[e, g, dim] + Reg[e_prima, g, dim])*14000/1e6)
    ...
    for dim in range(2):
        MODEL.addConstr(diforig[dim] >=  (datos.orig[dim] - xLo[1, dim])*14000/1e6)
        MODEL.addConstr(diforig[dim] >= (-datos.orig[dim] + xLo[1, dim])*14000/1e6)
    ...
    for o in O_index[:-1]:
        for dim in range(2):
            MODEL.addConstr(difLLo[o, dim] >=  (xLo[o, dim] - xLo[o+1, dim])*14000/1e6)
            MODEL.addConstr(difLLo[o, dim] >= (-xLo[o, dim] + xLo[o+1, dim])*14000/1e6)
    ...
    for o in O_index[:-1]:
        for dim in range(2):
            MODEL.addConstr(difLRo[o, dim] >=  (xLo[o, dim] - xRo[o+1, dim])*14000/1e6)
            MODEL.addConstr(difLRo[o, dim] >= (-xLo[o, dim] + xRo[o+1, dim])*14000/1e6)
    ...
    for o in O_index[:-1]:
        for dim in range(2):
            MODEL.addConstr(difRLo[o, dim] >=  (xRo[o, dim] - xLo[o+1, dim])*14000/1e6)
            MODEL.addConstr(difRLo[o, dim] >= (-xRo[o, dim] + xLo[o+1, dim])*14000/1e6)
    ...
    for o in O_index[:-1]:
        for dim in range(2):
            MODEL.addConstr(difRRo[o, dim] >=  (xRo[o, dim] - xRo[o+1, dim])*14000/1e6)
            MODEL.addConstr(difRRo[o, dim] >= (-xRo[o, dim] + xRo[o+1, dim])*14000/1e6)
    ...
    for dim in range(2):
        MODEL.addConstr(difdest[dim] >=  (xRo[O_index[-1], dim] - datos.dest[dim])*14000/1e6)
        MODEL.addConstr(difdest[dim] >= (-xRo[O_index[-1], dim] + datos.dest[dim])*14000/1e6)
    ...
    dM = (dorig + pLLo.sum('*') + pLRo.sum('*') + pRLo.sum('*') + pRRo.sum('*') + ddest)/datos.vC
    ...
    if not(datos.alpha):
        for g in G_index:
            MODEL.addConstr(gp.quicksum(peg[e, g]*grafos[g-1].longaristas[e // 100 - 1, e % 100] for e in grafos[g-1].aristas) == grafos[g-1].alpha*grafos[g-1].longitud)
