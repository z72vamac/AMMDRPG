...
def AMMDRPGST(datos): #, vals_xL, vals_xR):
    ...
    for t in T_index:
        # path_C.append([xLt[t, 0].X, xLt[t, 1].X])
        # path_C.append([xRt[t, 0].X, xRt[t, 1].X])
        plt.plot(xLt[t, 0].X, xLt[t, 1].X, 'ko', alpha = 1, markersize=10, color='blue')
        #ax.annotate("L" + str(t), xy = (xLt[t, 0].X, xLt[t, 1].X))
        plt.plot(xRt[t, 0].X, xRt[t, 1].X, 'ko', alpha = 1, markersize=10, color='blue')
    ...
    for pathd in paths_D:
        if esCerrado(pathd):
            # print(pathd)
            ax.add_artist(Polygon(pathd, fill=False, closed = True, lw = 2, alpha=1, color='red'))
        else:
            ax.add_artist(Polygon(pathd, fill=False, closed = False, lw = 2, alpha=1, color='red'))
