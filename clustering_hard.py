...
def clustering_hard(data, paths):
    """
    data: problem data,
    ...
    K_index = range(1, graphs_number + 1)
    G_index = K_index
    ...
    MODEL.addConstrs((prod_k_g.sum('*', g) + paths[g - 1][2] + prod_k_g_prime.sum('*', g)) / drone_speed <= time_endurance for g in G_index)
    ...
    # objective = gp.quicksum(dist_k_k[k1, k2] for k1, k2 in dist_k_k.keys()) + gp.quicksum(dist_k[k] for k in dist_k.keys())# + gp.quicksum(zk[k]*BigM for k in zk.keys())
    ...
    return MODEL.getAttr('x', C_k), MODEL.getAttr('x', assign_k_g)
