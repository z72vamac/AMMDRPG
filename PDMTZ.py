...
def PDMTZ(datos):
    ...
    if datos.init:
        try:
            z_init, xL_init, xR_init, results = heuristic(datos)
    ...
    for g, i in rhogi.keys():
        ...
        if mugi[g, i].X > 0.5:
            ...
            ax.annotate("$L^{{({0}, {1})_{2}}}$".format(first, second, g), xy = (Lgi[g, i, 0].X, Lgi[g, i, 1].X), fontsize = 11)
        if ugi[g, i].X > 0.5:
            ax.annotate("$R^{{({0}, {1})_{2}}}$".format(first, second, g), xy = (Rgi[g, i, 0].X, Rgi[g, i, 1].X), fontsize = 11)
    ...
    for pathd in paths_D:
        ax.add_artist(Polygon(pathd, fill=False, closed=False, lw = 2,
                      animated=False, alpha=1, color='orange'))
    #
    ...
    for g in range(nG):
        ...
        ax.annotate('$\\alpha_{0} = {1:0.2f}$'.format(g+1, grafo.alpha), xy = (centroide[0]+5, centroide[1]+10), fontsize = 12)
