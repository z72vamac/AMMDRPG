...
def aglomerativo(datos, paths):
    ...
    def canLink(list):
        ...
        MODEL.addConstrs(difLg[g, dim] >=  paths_problem[g][1][1][dim] - C[dim] for g, dim in difLg.keys())
        MODEL.addConstrs(difLg[g, dim] >= -paths_problem[g][1][1][dim] + C[dim] for g, dim in difLg.keys())
        MODEL.addConstrs(gp.quicksum(difLg[g, dim]*difLg[g, dim] for dim in range(2)) <= dLg[g]*dLg[g] for g in dLg.keys())
        
        MODEL.addConstrs(difRg[g, dim] >=  paths_problem[g][1][0][dim] - C[dim] for g, dim in difRg.keys())
        MODEL.addConstrs(difRg[g, dim] >= -paths_problem[g][1][0][dim] + C[dim] for g, dim in difRg.keys())
